...
L5 is a hard sci-fi 2D factory/logistics game (working title "L5") built on a custom game engine, targeting Linux, Windows, and Web. The dev environment is managed via Nix flakes — enter it with `nix develop` or `direnv allow`.
...
Build targets: Linux (`build/linux/`), Windows (`build/windows/`), and Web (`build/web/`).

## Architecture

- `art/` holds sprites/assets; `build/` is gitignored (build output only).
- 640×360 viewport with integer scaling.
